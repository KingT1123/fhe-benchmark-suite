...
Shrinking three of these chains for security compliance left too little coefficient-modulus budget to survive even a single homomorphic operation under BFV/BGV's standard plaintext modulus (SEAL's 20-bit batching prime, $t=1032193$ for $N \in \{2048, 4096, 8192\}$): fresh ciphertexts at these three configurations decrypted with zero remaining noise budget regardless of the operation performed. Each was instead given the smallest batching-compatible plaintext modulus that restored a real, positive noise-budget margin after the cell's actual required operation (Table~\ref{tab:configmeta-bfvbgv}).

\texttt{PlainModulus::Batching(N, 20)} is a search for a prime $t$ of at most 20 bits with $t \equiv 1 \pmod{2N}$, so the value it lands on depends on $N$: $t=1032193$ only holds for $N \in \{2048, 4096, 8192\}$. At $N=16384$ the same call, with the same congruence requirement evaluated against a larger $N$, lands on a different prime, $t=786433$ (confirmed directly against the built \texttt{SEALContext}, and independently verified: $786433 = 24 \times 32768 + 1$, i.e.\ $t \equiv 1 \pmod{2N}$ holds). This is not a defect and not one of the three exceptions above -- $N=16384$'s chains were never shrunk for noise-budget reasons -- it is simply the batching search landing on a different valid prime at a larger ring dimension. Results at $N=16384$ are unaffected and are kept, not dropped; this was a documentation gap (an inline comment and this section both stated the $N\le8192$ value as if it held everywhere), not an invalid result. See Table~\ref{tab:configmeta-bfvbgv} and \texttt{results/final/seal\_config\_metadata.csv} for the full recorded parameter facts, including this value, per cell.
...
\bottomrule
\end{tabular}
\caption{The three cells using a plaintext modulus other than the $N \le 8192$ standard $t=1032193$ (20-bit SEAL batching prime), and the resulting noise-budget margin after the cell's real required operation (add, since all three are depth-0). Necessary because their reduced coeff-modulus chains (Table~\ref{tab:paramgrid}) leave zero noise budget under the standard $t$ -- fresh ciphertexts otherwise fail to decrypt correctly at all. Every other $N \le 8192$ BFV/BGV cell in the grid is unaffected and keeps $t=1032193$; $N=16384$ is likewise unaffected by this exception but uses $t=786433$, the same 20-bit batching search's result at that $N$ (not a noise-budget fix -- see prose above).}
